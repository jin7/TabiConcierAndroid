    ...
    $('#map_canvas').gmap({
        ...
        'callback': function() {
            ...
            var control = self.get('control', function() {
                $(self.el).append("<div><a href='javascript:speechRecognize()' id='speechBtn' data-role='button' data-inline='true' data-iconpos='notext' data-icon='tabicon-speech'></a></div><div id='control'><div><input id='places-search' class='ui-bar-d ui-input-text ui-body-null ui-corner-all ui-shadow-inset ui-body-d ui-autocomplete-input' type='text'/></div></div>");
                return $('#control')[0];
            });
...
//        locObj.marker = marker;
    ...
    function addLocationToStorage(locObj)
    {
        ...
        var jsonObj = droid.stringify(locObj);
        if (jsonObj == null) alert("jsonObj is null");
        if (key == null) alert("key is null");
...
//    }
    ...
    {
    	var locObj = eval( "(" + localStorage.getItem(locKey, null) + ")" );
//        var locObj = $.jStorage.get(locKey);
        ...
        localStorage.setItem(locKey, droid.stringify(locObj));
...
//            var keys = $.jStorage.index();
            $.each(keys, function(i, k) {
                if (k != locKey) {
                	var locObj = eval( "(" + localStorage.getItem(k, null) + ")" );
//                    var locObj = $.jStorage.get(k);
                    locObj.isDept = false;
                    localStorage.setItem(k, droid.stringify(locObj));
//                    $.jStorage.set(k, locObj);
    ...
    function getKeys() {
    	var keys = [];
    	if (localStorage == null) alert("localStorage is null");
...
//        $.jStorage.deleteKey(locKey);
    ...
    function getLocationObject(latlng)
    {
        ...
        return eval( "(" + localStorage.getItem(locKey, null) + ")" );
...
//        });
        ...
        $(map).click( function(event) {
            // TabiconSpeechRecognizer.recog();
            // alert(TabiconSpeechRecognizer.getResult());
	...
	function speechRecognize() {
		droid.recognizeSpeech(speechRecognized, "ja");
	}
	function speechRecognized(str) {
        places_search_input.value += str;
	}

$(function(){
    $('#splash').click(function() {
        $.mobile.changePage("#mapview",{transition:"fade"});
        places_search_input.focus();
    });
//    setTimeout(function(){
//        $.mobile.changePage("#mapview",{transition:"fade"});
//    },3000);
